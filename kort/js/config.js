...
import { lagerNoegle, profil } from './profiles.js';

// Kortets centrum, zoom og ramme ligger nu i profiles.js — én profil pr. område.

// ─── Sentinel Hub ──────────────────────────────────────────────────────────────
export const SH_DEFAULT_INSTANCE_ID = profil.instansId || '';
export const SH_LS_KEY = lagerNoegle('sermilik_sh_instance_id');
...
// ─── GEBCO bathymetri ──────────────────────────────────────────────────────────
// Generic Bathymetric Chart of the Oceans — global havdybde-kort.
// WMS: https://wms.gebco.net/mapserv?
// Layers:
//   GEBCO_LATEST                 — havis-baseret + havdybde
//   GEBCO_LATEST_SUB_ICE_TOPO    — viser bathymetri UNDER Grønlands indlandsis (IceBridge BedMachine)
...
// ─── Localstorage-keys ─────────────────────────────────────────────────────────
export const DRAWN_LS_KEY = lagerNoegle('sermilik_drawings');
export const VALIDATION_LS_KEY = lagerNoegle('sermilik_validation_points');
...
// Samme Liang-formel, men farveskalaen er spredt over 0,00–0,45 i stedet for
// 0,15–0,85. Danske sommeroverflader ligger næsten alle under 0,30 — med
// is-skalaen ville hele landet rendere som én mørk flade. Trinene følger
// virkelige danske albedoer: vand 0,06 · asfalt 0,10–0,15 · mørke tage
// 0,10–0,20 · græs 0,20–0,25 · marker 0,15–0,30 · sand 0,30–0,40 · beton og
// lyse tage 0,30–0,50. Det hvide felt øverst er sne — så bliver vinteren
// pludselig pointen i det samme kort.
export const ALBEDO_EVALSCRIPT_DK = `//VERSION=3
function setup(){return{input:["B02","B04","B08","B11","B12","dataMask"],output:{bands:4}};}
function evaluatePixel(s){
  var a=0.356*s.B02+0.130*s.B04+0.373*s.B08+0.085*s.B11+0.072*s.B12-0.0018;
  var c;
  if(a<0.05)c=[0.06,0.14,0.24];
  else if(a<0.10)c=[0.11,0.29,0.39];
  else if(a<0.14)c=[0.18,0.50,0.55];
  else if(a<0.18)c=[0.35,0.66,0.61];
  else if(a<0.22)c=[0.58,0.77,0.56];
  else if(a<0.27)c=[0.82,0.85,0.54];
  else if(a<0.33)c=[0.94,0.84,0.49];
  else if(a<0.45)c=[0.97,0.92,0.82];
  else c=[1,1,1];
  return[c[0],c[1],c[2],s.dataMask];
}`;

// ─── Dataforsyningen (dansk ortofoto) ─────────────────────────────────────────
// På geo.sg.dk går kaldene gennem den cachende Apache-proxy, der allerede
// betjener landskabskortet — så flisene er ofte varme i RAM og koster intet.
// Lokalt (dev) rammer vi API'et direkte.
export const DF_TOKEN = 'c8725b58105e6e82957fe8042350c856';
export const DF_BASE = (typeof location !== 'undefined' && location.hostname === 'geo.sg.dk')
  ? '/landskab/df'
