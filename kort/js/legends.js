...
export const LEGENDS = {
  ...
  // Dansk albedoskala — samme Liang-formel, men trinene ligger hvor danske
  ...
  ALBEDO_DK: {
    ...
    description: 'Hvor meget af solens kortbølgede stråling overfladen kaster tilbage. '
      + 'Beregnet af fem Sentinel-2-bånd med Liang (2001): '
      + 'α = 0,356·B02 + 0,130·B04 + 0,373·B08 + 0,085·B11 + 0,072·B12 − 0,0018. '
      + 'Bemærk at hver værdi er et gennemsnit over 10 × 10 meter — asfalt, græs, '
      + 'tag og træer inden for samme felt lægges sammen til ét tal. Det er derfor '
      + 'jeres egen måling sjældent rammer præcis det samme.',
    stops: [
      { value: '> 0,45', label: 'sne', color: '#ffffff' },
      { value: '0,33', label: 'beton, lyse tage', color: '#f7ebd1' },
      { value: '0,27', label: 'tørre marker, sand', color: '#f0d67d' },
      { value: '0,22', label: 'lyse marker, stubmark', color: '#d1d98a' },
      { value: '0,18', label: 'græs', color: '#94c48f' },
      { value: '0,14', label: 'skov, mørke marker', color: '#59a89c' },
      { value: '0,10', label: 'asfalt', color: '#2e808c' },
      { value: '0,05', label: 'mørke tage, våd asfalt', color: '#1c4a63' },
      { value: '< 0,05', label: 'vand', color: '#0f243d' },
    ],
  },
...
export function renderLegend(layerId) {
  // Det danske kort bruger sin egen albedoskala — og skal have den forklaring med.
  if (layerId === 'ALBEDO' && profil.albedoSkala === 'dk') layerId = 'ALBEDO_DK';
